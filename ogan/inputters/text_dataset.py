...
from ogan.inputters.dataset_base import (DatasetBase, UNK_WORD,
                                         PAD_WORD, BOS_WORD, EOS_WORD)
# from tedm.utils.misc import aeq



class SingleTurnDataset(DatasetBase):
    def __init__(self, fields, src_examples_iter, tgt_examples_iter,
                 src_seq_length=0, tgt_seq_length=0,
                 use_filter_pred=True):
        self.data_type = "text_single_turn"
        ...
        super(SingleTurnDataset, self).__init__(out_examples, out_fields, filter_pred)
    ...
    @staticmethod
    def make_text_examples_tpl(text_iter, text_path, truncate, side):
        """
        Args:
        ...
        if text_iter is None:
            if text_path is not None:
                text_iter = SingleTurnDataset.make_text_iterator_from_file(text_path)
            else:
                return (None, 0)

        # All examples have same number of features, so we peek first one
        # to get the num_feats.
        examples_iter = SingleTurnDataset.make_examples(text_iter, truncate, side)
